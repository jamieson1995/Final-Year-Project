...
public abstract class Character {
	...
	protected bool m_stuckWithAFurniture;
			...
			if ( ( m_nextTile.m_furniture != null && m_nextTile.m_furniture.m_movable == true ) || m_movingFurniture != null )
			{
				...
				if ( m_pushingFurn == true )
				{
					...
					else
					{
						...
						{
							//toTile = FindValidPositionToMoveFurn ( m_currTile, m_nextTile, null );
							bool foundValidTile = false;
							List<Tile> invalidTiles = new List<Tile> ();
							invalidTiles.Add ( m_currTile );
							while ( foundValidTile == false )
	...
	public bool RequestInteraction ( Character _char, float _time, InteractionType _interType = InteractionType.General, Tile _tile = null )
	{
		...
		if ( _char.ReceiveInteraction ( this, _time, _interType, _tile ) )
		{
			...
			//Debug.Log(m_name + " and " + _char.m_name + " are interacting for " + _time + " seconds.");
	...
	{ 
		...
		if ( _amount < 0 )
		{
			//Debug.Log ( ID + " said something negative, relationship with " + _ID + " changed by " + _amount);
		}
		else if ( _amount > 0 )
		{
			//Debug.Log ( ID + " said something positive, relationship with " + _ID + " changed by " + _amount );
		}
		else if ( _amount == 0 )
		{
			//Debug.Log ( ID + " said something neutral to " + _ID );
		}
